use super::execution::{ComputeUnitLimits, PriorityFeeConfig};
use crate::wire::{
    helius_rpc::{HeliusPrioFeesHandle, PriorityLevel},
    jito::JitoClient,
};
...
impl SwapInstructionsBuilder {
    ...
    pub async fn handle_priority_fee_params(
        &mut self,
        priofees_handle: &Option<HeliusPrioFeesHandle>,
        ...
        match priority_fee_config {
            ...
            Some(PriorityFeeConfig::Dynamic) => {
                let cu_price = match priofees_handle
                    .as_ref()
                    .and_then(|handle| handle.get_priority_fee(PriorityLevel::High))
                {
                    Some(cu_price) => std::cmp::max(MIN_CU_PRICE, cu_price as u64),
                    None => MIN_CU_PRICE,
                };
                log::trace!("setting dynamic cu-price: cu-price={}", cu_price);
                let compute_ix =
                    solana_sdk::compute_budget::ComputeBudgetInstruction::set_compute_unit_price(
                        cu_price,
                    );
