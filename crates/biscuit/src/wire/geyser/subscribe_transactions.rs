use crate::core::traits::StreamInterface;
...
pub struct GrpcTransactions {
    ...
    receiver: tokio::sync::broadcast::Receiver<ITransaction>,
}

impl StreamInterface for GrpcTransactions {
    type Item = anyhow::Result<ITransaction>;

    fn drop_accounts(&self, requests: Vec<anchor_lang::prelude::Pubkey>) -> anyhow::Result<()> {
    ...
    fn subscribe(&self) -> futures::stream::BoxStream<'_, Self::Item> {
    ...
    fn watch_accounts(
        &self,
        requests: Vec<crate::core::traits::PollRequest>,
    ) -> anyhow::Result<()> {
        log::trace!("Got watch accounts request");
        let mut changed = false;
        for req in requests {
            if self.keys.insert(req.address) {
                changed = true;
            }
        }
...
fn grpc_watch_transactions_task_inner(
    ...
    task_name: String,
    txn_sender: tokio::sync::broadcast::Sender<ITransaction>,
                    ...
                    match update {
                        UpdateOneof::Transaction(update) => {
                            ...
                            if let Some(ref transaction) = update.transaction {
                                ...
                                let Ok(transaction) = ITransaction::try_from(update) else {
                                    log::error!(
                                        "grpc txns task: Failed to convert transaction {}",
                                        signature
                                    );
                                    continue;
                                };

                                if txn_sender.send(transaction).is_err() {
                                    error!(
